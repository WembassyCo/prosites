...
class RequestParser {
  ...
  public function __construct(
    ...
    $this->abjsConfig = $configFactory->get('abjs.settings');
  }
  ...
  /**
   * Generates the gatherer script.
   *
   * @return bool|string|string[]
   *   The script as a string, Drupal render array or FALSE if there's no test.
   */
  public function generateGathererScript() {
    $test = $this->getTest();

    // If no active test is found, don't do anything.
    if ($test === FALSE || !isset($test['tid'])) {
      return FALSE;
    }
