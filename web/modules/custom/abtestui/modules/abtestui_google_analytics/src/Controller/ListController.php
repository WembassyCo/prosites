...
class ListController extends OriginalListController {
  ...
  protected function testList() {
    ...
    foreach ($tests as $test) {
      if (!empty($test['analytics_url'])) {
        $prefield['analitycs_url'] = [
          ...
          'link' => [
            '#title' => $this->t('Analytics Results'),
            '#type' => 'link',
    ...
    if (empty($this->analyticsConfig->get('account'))) {
      $gaAccountId = '<a href="' . Url::fromRoute('google_analytics.admin_settings_form')
        ->toString() . '">' . $this->t('Set up') . '</a>';
    }
    else {
    ...
    $output['info_bar']['ga_check_results'] = [
      ...
      'link' => [
        '#title' => $this->t('How to check the results in Google Analytics'),
        '#type' => 'link',
